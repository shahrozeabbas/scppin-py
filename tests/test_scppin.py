import scppin
import pandas as pd


network = pd.read_csv('tests/raw_mg_ad_fava_string_network.tsv', sep='\t', index_col=0)
# network['merge_score'] = (network['fava_score'] + network['string_score']) / 2
network['merge_score'] = (network['fava_score'] * network['string_score'])
# network['merge_score'] = (network['fava_score'] * network['string_score']) ** 0.5


pvals_df = pd.read_csv('tests/magma_gene_symbol_results.tsv', sep='\t')
pvalues = dict(zip(pvals_df['GENE'], pvals_df['P']))

analyzer = scppin.scPPIN()
analyzer.load_network(network, weight_column='merge_score')

analyzer.set_node_weights(pvalues)
analyzer.detect_module(fdr=0.01, edge_weight_attr='weight', normalization=None, use_max_prize_root=True)


print(f"Module connected: {analyzer.module.is_connected()}")
print(f"Components: {len(analyzer.module.components())}")


print(f"\nModule nodes: {analyzer.module.vcount()}")
print(f"Module edges: {analyzer.module.ecount()}")
